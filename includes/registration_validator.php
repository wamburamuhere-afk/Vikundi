...
if (!function_exists('validate_registration_input')) {
     ...
     * @param bool   $requireEmail    Require an email. False for the registration
     *                                paths that auto-generate an identity email
     *                                (username@domain) when none is supplied.
     ...
     */
    function validate_registration_input(array $post, array $files, string $lang = 'en', bool $requireTerms = true, bool $requireSlip = true, bool $requirePassword = true, bool $requireEmail = true): array {
        $sw = ($lang === 'sw');
        $errors = [];
        ...
        if ($email === '') {
            ...
            // else: a blank email is allowed — an identity email (username@domain)
            // is auto-generated by the caller.
