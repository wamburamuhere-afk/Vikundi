...
$group_logo = $settings['group_logo'] ?? '';
...
// Workflow e-signatures
$wf_sigs = getWorkflowSignatures($pdo, 'contribution', $id);
$wf = [
...
$member_name = $con['customer_name'] ?: trim($con['first_name'] . ' ' . $con['last_name']);

logActivity('Viewed', 'Contributions', 'Printed Contribution #' . $id, 'CONTRIB#' . $id);
?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>Contribution #<?= $id ?> — <?= htmlspecialchars($group_name) ?></title>
<style>
    * { box-sizing: border-box; }
    ...
    .detail-card { border: 1px solid #e0e0e0; border-radius: 6px; padding: 10px 14px; }
    ...
    .amount-highlight { background: #f0fdf4; border-left: 4px solid #22c55e; padding: 10px 14px; border-radius: 4px; margin-bottom: 16px; display: flex; justify-content: space-between; align-items: center; }
    .amount-highlight .amt-label { font-size: 10px; text-transform: uppercase; color: #6c757d; font-weight: 700; }
    .amount-highlight .amt-val   { font-size: 22px; font-weight: 800; color: #15803d; }
...
<?php PrintHeader::render($pdo, 'CONTRIBUTION', 'REF #CONTRIB-' . $con['contribution_id']); ?>
...
<div class="amount-highlight">
    <div>
        <div class="amt-label">Contribution Amount</div>
        <div style="font-size:11px;color:#6c757d;margin-top:2px;"><?= htmlspecialchars($member_name) ?> &mdash; <?= date('d M Y', strtotime($con['contribution_date'])) ?></div>
    </div>
    <div class="amt-val"><?= $currency ?> <?= number_format($con['amount'], 2) ?></div>
</div>

<!-- Details grid -->
<div class="detail-grid">
    <div class="detail-card">
        <div class="dc-label">Member Name</div>
        <div class="dc-val"><?= htmlspecialchars($member_name) ?></div>
    </div>
    <div class="detail-card">
        <div class="dc-label">Contribution Date</div>
        <div class="dc-val"><?= date('d M Y', strtotime($con['contribution_date'])) ?></div>
    </div>
    <div class="detail-card">
        <div class="dc-label">Contribution Type</div>
        <div class="dc-val"><?= ucfirst($con['contribution_type'] ?? 'monthly') ?></div>
    </div>
    <div class="detail-card">
        <div class="dc-label">Phone</div>
        <div class="dc-val"><?= htmlspecialchars($con['phone'] ?? '—') ?></div>
    </div>
    <?php if (!empty($con['description'])): ?>
    <div class="detail-card" style="grid-column:1/-1;">
        <div class="dc-label">Description</div>
        <div class="dc-val"><?= htmlspecialchars($con['description']) ?></div>
    </div>
    <?php endif; ?>
    <?php if (!empty($con['mkoba_receipt'])): ?>
    <div class="detail-card">
        <div class="dc-label">Mkoba Receipt</div>
        <div class="dc-val"><?= htmlspecialchars($con['mkoba_receipt']) ?></div>
    </div>
    <?php endif; ?>
</div>
