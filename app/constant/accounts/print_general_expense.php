...
$wf_sigs = getWorkflowSignatures($pdo, 'general_expense', $id);
$wf = [
...
logActivity('Viewed', 'Other Expenses', 'Printed General Expense #' . $id, 'GE#' . $id);
?>
<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<title>Expense #<?= $id ?> — <?= htmlspecialchars($group_name) ?></title>
<style>
    * { box-sizing: border-box; }
    ...
    .amount-highlight { background: #fffbf0; border-left: 4px solid #ffc107; padding: 10px 14px; border-radius: 4px; margin-bottom: 16px; display: flex; justify-content: space-between; align-items: center; }
    .amount-highlight .amt-label { font-size: 10px; text-transform: uppercase; color: #6c757d; font-weight: 700; }
    .amount-highlight .amt-val   { font-size: 22px; font-weight: 800; color: #856404; }
...
<?php PrintHeader::render($pdo, 'GENERAL EXPENSE VOUCHER', 'REF #GE-' . $ge['id']); ?>

<div class="status-strip">Status: <?= strtoupper($status) ?></div>

<div class="amount-highlight">
    <div>
        <div class="amt-label">Expense Amount</div>
        <div style="font-size:11px;color:#6c757d;margin-top:2px;"><?= htmlspecialchars($ge['description'] ?? '') ?> &mdash; <?= date('d M Y', strtotime($ge['expense_date'])) ?></div>
    </div>
    <div class="amt-val"><?= $currency ?> <?= number_format($ge['amount'], 2) ?></div>
</div>

<div class="detail-grid">
    <div class="detail-card">
        <div class="dc-label">Description</div>
        <div class="dc-val"><?= safe_output($ge['description']) ?></div>
    </div>
    <div class="detail-card">
    ...
    <div class="detail-card">
        ...
        <div class="dc-val"><?= safe_output($ge['category']) ?></div>
    </div>
    <?php endif; ?>
</div>
...
.three-approval-watermark {
    ...
    color:rgba(255,193,7,0.22); transform:rotate(-30deg);
